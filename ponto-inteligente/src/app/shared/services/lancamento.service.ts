...
export class LancamentoService {
  ...
  private readonly PATH_ULTIMO_LANCAMENTO: string = '/funcionario/{funcionarioId}/ultimo';
  ...
  buscarUltimoLancamento(): Observable<any> {
    return this.http.get(
      env.baseApiUrl + this.PATH + 
        this.PATH_ULTIMO_LANCAMENTO.replace('{funcionarioId}', this.httpUtil.obterIdUsuario()), 
  ...
  listarTodosLancamentos(): Observable<any> {
    return this.http.get(
        env.baseApiUrl + this.PATH + 
          this.PATH_TODOS_LANC.replace('{funcionarioId}', this.httpUtil.obterIdUsuario()),
        this.httpUtil.headers()      
    );
  }
  ...
  atualizar(lancamento: Lancamento): Observable<any> {
    return this.http.put(env.baseApiUrl + this.PATH + '/' + lancamento.id, 
      lancamento,
      this.httpUtil.headers());
  }
}
